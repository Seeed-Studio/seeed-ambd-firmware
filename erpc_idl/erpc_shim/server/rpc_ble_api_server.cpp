/*
 * Generated by erpcgen 1.7.4 on Wed Aug 19 10:27:01 2020.
 *
...
erpc_status_t rpc_gap_bone_service::rpc_le_bond_get_display_key_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    key = (binary_t *) erpc_malloc(sizeof(binary_t));
    if (key == NULL)
    {
        codec->updateStatus(kErpcStatus_MemoryError);
    }
    ...
    if (!err)
    {
        ...
        write_binary_t_struct(codec, key);
...
erpc_status_t rpc_gap_le_service::rpc_le_set_host_chann_classif_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    binary_t *p_channel_map = NULL;
    p_channel_map = (binary_t *) erpc_malloc(sizeof(binary_t));
    ...
    read_binary_t_struct(codec, p_channel_map);
    ...
    if (p_channel_map)
    {
        free_binary_t_struct(p_channel_map);
    }
    if (p_channel_map)
    {
        erpc_free(p_channel_map);
    }
...
erpc_status_t rpc_gap_conn_service::rpc_le_connect_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    uint8_t init_phys;
    uint8_t remote_bd[6];
    RPC_T_GAP_CONN_PARAM_TYPE remote_bd_type;
    ...
    codec->read(&_tmp_local);
    remote_bd_type = static_cast<RPC_T_GAP_CONN_PARAM_TYPE>(_tmp_local);
