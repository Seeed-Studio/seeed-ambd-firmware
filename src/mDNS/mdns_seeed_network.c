...
static esp_err_t _udp_pcb_main_init(void)
{
    ...
    ip_addr_copy(_pcb_main->remote_ip, ip_addr_any);
...
{
    struct netif * netif = &xnetif[if_inx];
    ...
    if (ip_protocol == MDNS_IP_PROTOCOL_V4) {
        ...
        // if(join){
        //     if (mld6_joingroup_netif(netif, &(multicast_addr.addr))) {
        //         return ESP_ERR_INVALID_STATE;
        //     }
        // } else {
        //     if (mld6_leavegroup_netif(netif, &(multicast_addr.addr))) {
        //         return ESP_ERR_INVALID_STATE;
        //     }
        // }
    }
...
static void _udp_recv(void *arg, struct udp_pcb *upcb, struct pbuf *pb, const ip_addr_t *raddr, uint16_t rport)
{
    ...
    while (pb != NULL) {
        ...
        for (i=0; i<MDNS_IF_MAX; i++) {
            ...
            netif = &xnetif[_mdns_get_esp_netif(i)];
        ...
        if (!pcb || !_mdns_server || !_mdns_server->action_queue || _mdns_send_rx_action(packet) != ESP_OK) {
            pbuf_free(this_pb);
            free(packet);
        }
    }
...
static err_t _mdns_udp_pcb_write_api(struct tcpip_api_call_data *api_call_msg)
{
    ...
    nif = (void *)&xnetif[_mdns_get_esp_netif(msg->tcpip_if)];
    if (!nif) {
        pbuf_free(msg->pbt);
        msg->err = ERR_IF;
        return ERR_IF;
...
size_t _mdns_udp_pcb_write(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, const ip_addr_t *ip, uint16_t port, uint8_t * data, size_t len)
{
