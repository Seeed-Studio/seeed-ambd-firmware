...
static volatile TaskHandle_t _mdns_timer_task_handle = NULL;
static volatile TaskHandle_t _mdns_scan_task_handle = NULL;
...
static void _mdns_search_result_add_ip(mdns_search_once_t * search, const char * hostname, ip_addr_t * ip, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol);
 ...
 */
mdns_if_t _mdns_get_esp_netif(mdns_if_t tcpip_if)
{
 ...
 */
static mdns_if_t _mdns_get_if_from_esp_netif(mdns_if_t tcpip_if)
{
    if (tcpip_if < MDNS_IF_MAX) {
        return tcpip_if;
    }
...
{
    if (service && !_str_null_or_empty(service->instance_test)) {
        return service->instance_test;
    }
...
static uint8_t _mdns_append_answer(uint8_t * packet, uint16_t * index, mdns_out_answer_t * answer, mdns_if_t tcpip_if)
{
    if (answer->type == MDNS_TYPE_PTR) {
        ...
        // if(!netif_is_up(&xnetif[tcpip_if])){
        //     return 0;
        // }
...
static void _mdns_init_pcb_probe_new_service(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t ** services, size_t len, bool probe_ip)
{
    ...
    _mdns_schedule_tx_packet(packet, ((pcb->failed_probes > 5)?1000:120) + (rand() & 0x7F));
...
static void _mdns_init_pcb_probe(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t ** services, size_t len, bool probe_ip)
{
    ...
    if (PCB_STATE_IS_PROBING(pcb)) {
        ...
        _mdns_init_pcb_probe_new_service(tcpip_if, ip_protocol,new_probe_service_len?new_probe_services:NULL, new_probe_service_len, probe_ip);
    } else {
...
static mdns_txt_linked_item_t * _mdns_allocate_txt(size_t num_items, mdns_txt_item_t txt[])
{
    ...
    if (num_items) {
        for (i=0; i<num_items; i++) {
            ...
            new_item->key = NULL;
            new_item->key = malloc(MDNS_NAME_BUF_LEN);
            if(new_item->key != NULL){
                strncpy(new_item->key,txt[i].key,MDNS_NAME_BUF_LEN);
            }
            if (!new_item->key) {
                free(new_item);
                break;
            ...
            new_item->value = NULL;
            new_item->value = malloc(MDNS_NAME_BUF_LEN);
            if(new_item->value != NULL){
                strncpy(new_item->value,txt[i].value,MDNS_NAME_BUF_LEN);
            }
            if (!new_item->value) {
...
static mdns_service_t * _mdns_create_service(const char * service, const char * proto, uint16_t port, const char * instance, size_t num_items, mdns_txt_item_t txt[])
{
    ...
    s->instance_test = NULL;
    if(instance != NULL){
        s->instance_test = malloc(MDNS_NAME_BUF_LEN);
        if(s->instance_test != NULL){
            strncpy(s->instance_test,instance,MDNS_NAME_BUF_LEN - 1);
        }
    }
    ...
    s->service = NULL;
    if(service != NULL){
        s->service = malloc(MDNS_NAME_BUF_LEN);
        if(s->service != NULL){
            strncpy(s->service,service,MDNS_NAME_BUF_LEN - 1);
        }
    }

    if (!s->service) {
        if(s->instance_test != NULL){
           free(s->instance_test); 
        }
        free(s);
        return NULL;
    }
    ...
    s->proto = NULL;
    if(proto != NULL){
        s->proto = malloc(MDNS_NAME_BUF_LEN);
        if(s->proto != NULL){
            strncpy(s->proto,proto,MDNS_NAME_BUF_LEN - 1);
        }
    }
...
static void _mdns_free_service(mdns_service_t * service)
{
    ...
    free((char *)service->instance_test);
 ...
 */
static int _mdns_check_a_collision(ip_addr_t * ip, mdns_if_t tcpip_if)
{
    ...
    //     return 1;//they win
    // }
    if (xnetif[tcpip_if].ip_addr.addr) {
        return 1;//they win
    }

    int ret = memcmp((uint8_t*)&if_ip_info.ip.addr, (uint8_t*)&ip->addr, sizeof(ip_addr_t));
    if (ret > 0) {
        return -1;//we win
    } else if (ret < 0) {
...
static esp_err_t _mdns_strdup_check(char ** out, char * in)
{
    if (in && in[0]) {
        *out = NULL;
        *out = malloc(MDNS_NAME_BUF_LEN);
        if(*out != NULL){
            strncpy(*out,in,MDNS_NAME_BUF_LEN);
        }
        if (!*out) {
            return ESP_FAIL;
        }
...
void mdns_parse_packet(mdns_rx_packet_t * packet)
{
    ...
    parsed_packet->src.addr = packet->src.addr;
        ...
        while (qs--) {
            ...
            if (_mdns_name_is_discovery(name, type)) {
                ...
                while (a) {
                    ...
                    question->service = malloc(MDNS_NAME_BUF_LEN);
                    if(question->service != NULL){
                        strncpy(question->service,a->service->service,MDNS_NAME_BUF_LEN);
                    }

                    question->proto = NULL;
                    question->proto = malloc(MDNS_NAME_BUF_LEN);
                    if(question->proto != NULL){
                        strncpy(question->proto,a->service->proto,MDNS_NAME_BUF_LEN);
                    }

                    question->domain = NULL;
                    question->domain = malloc(MDNS_NAME_BUF_LEN);
                    if(question->domain != NULL){
                        strncpy(question->domain,MDNS_DEFAULT_DOMAIN,MDNS_NAME_BUF_LEN);
                    }
                ...
                if (search_result) {
                    if (search_result->type == MDNS_TYPE_PTR) {
                        if (!result->hostname) { // assign host/port for this entry only if not previously set
                            ...
                            result->hostname = malloc(MDNS_NAME_BUF_LEN);
                            if(result->hostname != NULL){
                                strncpy(result->hostname,name->host,MDNS_NAME_BUF_LEN);
                            }
                        }
                    } else {
                    ...
                    if (col && (parsed_packet->probe || parsed_packet->authoritative)) {
                        if (col > 0 || !port) {
                            ...
                            if (_mdns_server->interfaces[packet->tcpip_if].pcbs[packet->ip_protocol].probe_running) {
                                ...
                                if (!_str_null_or_empty(service->service->instance_test)) {
                                    char * new_instance = _mdns_mangle_name((char *)service->service->instance_test);
                                    if (new_instance) {
                                        free((char *)service->service->instance_test);
                                        service->service->instance_test = new_instance;
                                    }
 ...
 */
static void _mdns_handle_system_event(esp_event_base_t event_base,int32_t event_id, mdns_if_t interface)
{
    if (!_mdns_server) {
        return;
    }
...
static mdns_search_once_t * _mdns_search_init(const char * name, const char * service, const char * proto, uint16_t type, uint32_t timeout, uint8_t max_results)
{
    ...
    if (!_str_null_or_empty(name)) {
        ...
        search->instance = malloc(MDNS_NAME_BUF_LEN);
        if(search->instance != NULL){
            strncpy(search->instance,name,MDNS_NAME_BUF_LEN - 1);
        }
    ...
    if (!_str_null_or_empty(service)) {
        search->service = NULL;
        search->service = malloc(MDNS_NAME_BUF_LEN);
        if(search->service != NULL){
            strncpy(search->service,service,MDNS_NAME_BUF_LEN - 1);
        }
    ...
    if (!_str_null_or_empty(proto)) {
        search->proto = NULL;
        search->proto = malloc(MDNS_NAME_BUF_LEN);
        if(search->proto != NULL){
            strncpy(search->proto,proto,MDNS_NAME_BUF_LEN - 1);
        }
...
static void _mdns_search_result_add_ip(mdns_search_once_t * search, const char * hostname, ip_addr_t * ip, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol)
{
    ...
    if ((search->type == MDNS_TYPE_A) || search->type == MDNS_TYPE_ANY) {
        r = search->result;
        while (r) {
...
static mdns_result_t * _mdns_search_result_add_ptr(mdns_search_once_t * search, const char * instance, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol)
{
    ...
    if (!search->max_results || search->num_results < search->max_results) {
        ...
        r->instance_name = malloc(MDNS_NAME_BUF_LEN);
        if(r->instance_name != NULL){
            strncpy(r->instance_name,instance,MDNS_NAME_BUF_LEN);
        }
...
static void _mdns_search_result_add_srv(mdns_search_once_t * search, const char * hostname, uint16_t port, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol)
{
    ...
    if (!search->max_results || search->num_results < search->max_results) {
        ...
        memset(r, 0 , sizeof(mdns_result_t));
        r->hostname = NULL;
        r->hostname = malloc(MDNS_NAME_BUF_LEN);
        if(r->hostname != NULL){
            strncpy(r->hostname,hostname,MDNS_NAME_BUF_LEN);
        }
...
static void _mdns_execute_action(mdns_action_t * action)
{
    ...
    case ACTION_SYSTEM_EVENT:
        // _mdns_handle_system_event(action->data.sys_event.event_base,action->data.sys_event.event_id, action->data.sys_event.interface);
        _mdns_handle_system_event(action->data.sys_event.event_base,action->data.sys_event.event_id, action->data.sys_event.netif_id);
        break;
    case ACTION_HOSTNAME_SET:
    ...
    case ACTION_SERVICE_ADD:
        ...
        break;
    case ACTION_SERVICE_INSTANCE_SET:
        if (action->data.srv_instance.service->service->instance_test) {
            _mdns_send_bye(&action->data.srv_instance.service, 1, false);
            free((char*)action->data.srv_instance.service->service->instance_test);
        }
        action->data.srv_instance.service->service->instance_test = action->data.srv_instance.instance;
...
// static void _mdns_scan_cb(void * arg)
// {
//     while(1){
//         if(results){
...
static esp_err_t _mdns_start_timer(void){
    ...
    if (!_mdns_timer_task_handle) {
        xTaskCreate(_mdns_timer_cb, "mdns_timer_task", 512, NULL, MDNS_TASK_PRIORITY,(TaskHandle_t * const)(&_mdns_timer_task_handle));
    }

    if(!_mdns_scan_task_handle){
        // xTaskCreate(_mdns_scan_cb, "mdns_scan_task", 1024, NULL, MDNS_TASK_PRIORITY,(TaskHandle_t * const)(&_mdns_scan_task_handle));
    }
...
static esp_err_t _mdns_service_task_start(void)
{
    ...
    if (!_mdns_service_task_handle) {
        xTaskCreate(_mdns_service_task, "mdns", MDNS_SERVICE_STACK_DEPTH, NULL, MDNS_TASK_PRIORITY,(TaskHandle_t * const)(&_mdns_service_task_handle));
        if (!_mdns_service_task_handle) {
            _mdns_stop_timer();
            MDNS_SERVICE_UNLOCK();
...
void mdns_pcb_enable(char *buf, int buf_len, int flags, void *userdata)
{
    int i = 0;
    tcpip_adapter_ip_info_t if_ip_info;

    for (i=0; i<MDNS_IF_MAX; i++) {
        if (netif_is_up(&xnetif[i]) && !tcpip_adapter_get_ip_info(i,&if_ip_info) && if_ip_info.ip.addr) {
            _mdns_enable_pcb(i, MDNS_IP_PROTOCOL_V4);
        }
    }
}

void mdns_pcb_disable(char *buf, int buf_len, int flags, void *userdata)
{
    int i = 0;
...
esp_err_t mdns_init(void)
{
    ...
    wifi_reg_event_handler(WIFI_EVENT_CONNECT, mdns_pcb_enable, NULL);
    wifi_reg_event_handler(WIFI_EVENT_STA_ASSOC, mdns_pcb_enable, NULL);
    wifi_reg_event_handler(WIFI_EVENT_IP_CHANGED, mdns_pcb_enable, NULL);
    wifi_reg_event_handler(WIFI_EVENT_DISCONNECT, mdns_pcb_disable, NULL);
    wifi_reg_event_handler(WIFI_EVENT_STA_DISASSOC, mdns_pcb_disable, NULL);
...
free_event_handlers:
    ...
    wifi_unreg_event_handler(WIFI_EVENT_CONNECT, mdns_pcb_enable);
    wifi_unreg_event_handler(WIFI_EVENT_STA_ASSOC, mdns_pcb_enable);
    wifi_unreg_event_handler(WIFI_EVENT_IP_CHANGED, mdns_pcb_enable);
    wifi_unreg_event_handler(WIFI_EVENT_DISCONNECT, mdns_pcb_disable);
    wifi_unreg_event_handler(WIFI_EVENT_STA_DISASSOC, mdns_pcb_disable);
...
esp_err_t mdns_hostname_set(const char * hostname)
{
    ...
    char * new_hostname = (char *)malloc(MDNS_NAME_BUF_LEN);
    if (!new_hostname) {
        return ESP_ERR_NO_MEM;
    }
    strncpy(new_hostname,hostname,MDNS_NAME_BUF_LEN - 1);
...
esp_err_t mdns_instance_name_set(const char * instance)
{
    ...
    new_instance = malloc(MDNS_NAME_BUF_LEN);
    if(new_instance != NULL){
        strncpy(new_instance,instance,MDNS_NAME_BUF_LEN - 1);
    }
...
esp_err_t mdns_service_txt_item_set(const char * service, const char * proto, const char * key, const char * value)
{
    ...
    action->data.srv_txt_set.key = NULL;
    if(key != NULL){
        int size = strlen(key);
        action->data.srv_txt_set.key = malloc(size+1);
        if(action->data.srv_txt_set.key != NULL){
            strncpy(action->data.srv_txt_set.key,key,size);
            action->data.srv_txt_set.key[size] = '\0';
        }
    }
    ...
    if(value != NULL){
        ...
        action->data.srv_txt_set.value = malloc(size);
        if(action->data.srv_txt_set.value != NULL){
            strncpy(action->data.srv_txt_set.value,value,size);
            action->data.srv_txt_set.value[size] = '\0';
        }
    }
...
esp_err_t mdns_service_txt_item_remove(const char * service, const char * proto, const char * key)
{
    ...
    action->data.srv_txt_del.key = NULL;
    action->data.srv_txt_del.key = malloc(MDNS_NAME_BUF_LEN);
    if(action->data.srv_txt_del.key != NULL){
        strncpy(action->data.srv_txt_del.key,key,MDNS_NAME_BUF_LEN);
    }
...
esp_err_t mdns_query_a(const char * name, uint32_t timeout, ip_addr_t * addr)
{
    mdns_result_t * result = NULL;
    esp_err_t err;
    ...
    if (header.answers || header.servers || header.additional) {
        ...
        while (content < (data + len)) {
            ...
            if (type == MDNS_TYPE_PTR) {
                ...
                ip_addr_t ip;
                memcpy(&ip, data_ptr, sizeof(ip_addr_t));
